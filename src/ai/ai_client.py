    ...
    def send_query(self, user_text: str) -> None:
        ...
        if user_safety.flagged:
            if self._event_bus:
                self._event_bus.emit(
                    ...
                    "User input flagged for self-harm signals",
                )
            self._history.append({"role": "user", "content": user_text})
            self._trim_history()
            crisis_text = crisis_response()
            if self._event_bus:
                self._event_bus.emit("response_chunk", crisis_text)
            self._history.append({"role": "assistant", "content": crisis_text})
            self._trim_history()
            if self._event_bus:
                self._event_bus.emit("response_done", crisis_text)
            return
        ...
        if full_response:
            ...
            if response_safety.flagged:
                ...
                if self._event_bus:
                    self._event_bus.emit(
                        "safety_event", category_value, response_safety.message
                    )
