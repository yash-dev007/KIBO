...
class ProactiveEngine(QObject):
    ...
    def __init__(self, config: dict, router: NotificationRouter, task_runner=None) -> None:
        ...
        # Derive phase from config
        cal = config.get("calendar_provider", "none")
        has_calendar = cal != "none"
        self._phase: int = 3 if has_calendar else (2 if config.get("proactive_enabled", True) else 1)

        # Sync task state from disk if task_runner provided
